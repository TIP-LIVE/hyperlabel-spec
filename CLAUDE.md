...
### QR Scanning (Photo-Only)

QR scanning is the most-fought UI flow in the app — labels are printed in green-on-navy, which collapses to similar luminance values and breaks live-camera decoders. **The app no longer does live continuous scanning.** Every scanner UI takes a single still photo and decodes it through a two-stage pipeline.

#### Why photo-only

Live continuous scanning was tried with parallel decoders (native BarcodeDetector + channel extraction, Nimiq qr-scanner, zxing) and never got above ~80% scan success on real labels. The still-photo path runs a much heavier preprocessing pipeline (~500ms server side) that simply wouldn't fit in a 16ms video frame budget. Admin flows standardized on photo-only by April 2026; the user-facing flow followed shortly after, and `startQrScan` / `@zxing` were removed entirely.

#### Photo decode pipeline (`decodeQrFromImage` in `lib/qr-scan.ts`)

Two stages, escalates only on miss:

1. **Client `BarcodeDetector`** on a 1500px downscale — fast, ~50ms, covers normal-contrast prints.
2. **Server `/api/v1/decode-qr`** (ZBar WASM) — 7-strategy preprocessing pipeline (channel extractions for green-on-navy + grayscale variants). Big-guns endpoint, ~500ms+, only invoked when stage 1 misses.

A previous Nimiq `qr-scanner` middle stage was removed once the ZBar pipeline subsumed its wins; if you're considering reintroducing a third client-side decoder, run an A/B with logs first — it didn't pull weight last time.

#### Camera capture UX (`PhotoCaptureButton`)

The shared `PhotoCaptureButton` (`components/shared/photo-capture-button.tsx`) handles both platforms:

- **Mobile** (`isMobileDevice()` check) — `<input type="file" accept="image/*" capture="environment">` opens the system camera. Native UIs are higher resolution and more reliable than in-app webcams.
- **Desktop** — opens a `WebcamSnapshotDialog` with live preview + manual shutter button (uses `startWebcamPreview` + `captureSnapshot`). The `capture` attribute silently falls back to a file picker on desktop, which is a poor UX, hence the dialog.

#### Critical rules

1. **Don't reintroduce live continuous scanning.** It was removed because it never worked reliably on green-on-navy prints, not because it was inconvenient. If you're tempted to add `decodeFromStream` or a per-frame requestAnimationFrame loop, stop and re-read this section.
2. **Use `PhotoCaptureButton` for any new scanner UI.** It handles the mobile/desktop split correctly — don't roll your own `<input capture>` button or you'll regress the desktop UX.
3. **Server escalation is slow on cold start** — the ZBar WASM endpoint can take several seconds for the first request after a Vercel deploy. Surface a "first request after deploy can cold-start" hint in error messages so users know to retry.
4. **Print quality is the upstream problem.** Andrii's note (Apr 2026): white-ink thermal printers are expensive and hard to source, hence the green-on-navy. If print contrast improves (white-ink upgrade, neon-on-black stock with black thermal print), drop pipeline complexity rather than adding more decoder stages.

#### Files

| File | Role |
|------|------|
| `app/src/lib/qr-scan.ts` | Photo decode pipeline + webcam preview/snapshot helpers |
...
| `app/src/components/shared/photo-capture-button.tsx` | Mobile/desktop camera capture button (used by all scanner UIs) |
...
| `app/src/components/dispatch/label-scan-dialog.tsx` | Admin dispatch label scanner UI |
...
15. **Don't reintroduce live continuous QR scanning** — it was removed in April 2026 because it never got above ~80% on green-on-navy prints. All scanners take a single still photo and run a heavier server-side pipeline. If you find yourself adding `decodeFromStream` or a per-frame `requestAnimationFrame` decode loop, stop and re-read the QR Scanning section
16. **Use `PhotoCaptureButton` for any new scanner UI** — it handles the mobile (`<input capture>`) vs desktop (webcam preview dialog) split. Rolling your own `<input capture>` button regresses desktop UX

 ...
 * preferred because callers often invoke `startWebcamPreview` in the same
 * tick as the state change that mounts the <video> (e.g. switching dialog
...
// Live continuous scanning (`startQrScan`) was removed — green-on-navy
// printed labels are too low-contrast for any decoder to hit reliably at
// video frame rate, and users mostly fell back to the photo upload path
